...
static VOS_STATUS hdd_apply_cfg_ini( hdd_context_t *pHddCtx, tCfgIniEntry* iniTable, unsigned long entries)
{
   ...
   for ( idx = 0; idx < cRegTableEntries; idx++, pRegEntry++ )
   {
           ...
           ( WLAN_PARAM_HexInteger == pRegEntry->RegType ) )
      {
         ...
         if (hdd_get_conparam() == VOS_POWERSAVE_OFF)
         {
            if (strcmp(pRegEntry->RegName, "gEnableImps") == 0){
                value=0;
                pr_info("chq: %s=%u\n", pRegEntry->RegName, value);
            }
            if (strcmp(pRegEntry->RegName, "gEnableBmps") == 0){
                value=0;
                pr_info("chq: %s=%u\n", pRegEntry->RegName, value);
            }
            if (strcmp(pRegEntry->RegName, "FastTransitionEnabled") == 0){
                value=0;
                pr_info("chq: %s=%u\n", pRegEntry->RegName, value);
            }
            if (strcmp(pRegEntry->RegName, "FastRoamEnabled") == 0){
                value=0;
                pr_info("chq: %s=%u\n", pRegEntry->RegName, value);
            }
        }
