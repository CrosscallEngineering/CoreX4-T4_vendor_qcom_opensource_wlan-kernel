...
/*hisense add for dync changer mipi freq */
#ifdef CONFIG_CHANGE_MIPI_CLK_FOLLOW_WIFI
#define WIFI_INTERFERE_CHANNEL1	100

/**
 * set channel_id when sta/ap/p2p is connected
 * @chan_id: chan_id
 *
 * Return: none
 */
int current_chanid = 0;
void wifi_dynamic_mipiclk(int chan_id)
{
	current_chanid=chan_id;
}
EXPORT_SYMBOL_GPL(wifi_dynamic_mipiclk);
#endif
...
void hdd_exchange_version_and_caps(hdd_context_t *pHddCtx)
{
   ...
   do {
	  ...
	  vos_nv_readDefaultCountryTable( &nvTables );
	  vos_mem_copy(country_name, &nvTables.defaultCountryTable.countryCode,2);
	  wcnss_get_iris_name(wcnss_id_name);
	  snprintf(wlan_version1,120,"IC:%s,driver version %s firmware version %s country code: %c%c",wcnss_id_name,QWLAN_VERSIONSTR,versionString,country_name[0],country_name[1]);
	  productinfo_register(PRODUCTINFO_WIFI_ID, wlan_version1, NULL);
