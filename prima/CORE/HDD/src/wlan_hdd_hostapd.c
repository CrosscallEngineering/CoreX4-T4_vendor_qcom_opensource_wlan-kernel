...
extern void wifi_dynamic_mipiclk(int chan_id);
#endif
...
VOS_STATUS hdd_hostapd_SAPEventCB( tpSap_Event pSapEvent, v_PVOID_t usrDataForCallback)
{
    ...
    switch(sapEvent)
    {
        case eSAP_START_BSS_EVENT :
			...
			#ifdef CONFIG_CHANGE_MIPI_CLK_FOLLOW_WIFI
			if(!( pSapEvent->sapevt.sapStartBssCompleteEvent.status) )
				wifi_dynamic_mipiclk(pSapEvent->sapevt.sapStartBssCompleteEvent.operatingChannel);
			#endif
